...
public class Rs232FiscalPrinterInfoHelper implements FiscalPrinterInfoHelper {
   ...
   static Rs232FiscalPrinterInfoHelper instance = null;
   boolean countrySet = false;
   int country = 0;
   int version = 0;
   boolean powerInterrupted = false;

   private Rs232FiscalPrinterInfoHelper() {
   }

   public static Rs232FiscalPrinterInfoHelper getInstance() {
      if (instance == null) {
         instance = new Rs232FiscalPrinterInfoHelper();
      }
   ...
   public boolean isSuccessfulCommand(byte[] data) {
      return this.getReturnCode(data) == 67 || (data[8] & 8) == 8;
   }

   public int getCountry() {
      while (!this.countrySet) {
         try {
            Thread.sleep(50L);
